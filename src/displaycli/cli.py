...
def list_displays(args):
    try:
        ...
        logger.log("Available Displays:", level="INFO")
        ...
        logger.log(f"Error listing displays: {e}", level="CRITICAL")
        sys.exit(1)
...
def list_modes(args):
    try:
        ...
        if not modes:
            logger.log(f"No modes found for display ID {display_id}, or display ID is invalid.", level="WARN")
            return
        ...
        if args.aspect_ratio:
            ...
            if current_mode:
                ...
                logger.log(f"Filtering for aspect ratio: {target_aspect_ratio[0]}:{target_aspect_ratio[1]}", level="INFO")
        
        logger.log(f"Available modes for display ID {display_id}:", level="INFO")
        ...
        if count == 0: logger.log("No matching modes found.", level="INFO")

    except ValueError:
        logger.log(f"Invalid display ID: {args.display_id}. Please provide a number.", level="ERROR")
        sys.exit(1)
    except Exception as e:
        logger.log(f"Error listing modes: {e}", level="CRITICAL")
        sys.exit(1)
...
def set_mode(args):
    try:
        ...
        logger.log(f"Attempting to set display {display_id} to {width}x{height} @ {refresh if refresh > 0 else 'any'}Hz", level="INFO")
        mode_to_set = manager.find_mode(display_id, width, height, refresh)

        if not mode_to_set:
            logger.log(f"Could not find a matching mode.", level="ERROR")
            sys.exit(1)

        answer = logger.prompt("This will change your display resolution. Are you sure? [y/N]", level="WARN")
        if answer.lower() != 'y':
            logger.log("Operation cancelled by user.", level="INFO")
            return

        manager.set_display_mode(display_id, mode_to_set)
        logger.log("Display mode changed successfully!", level="INFO")

    except ValueError:
        logger.log("Invalid arguments. Please ensure all IDs, dimensions are numbers.", level="ERROR")
        sys.exit(1)
    except RuntimeError as e:
        logger.log(f"Failed to set display mode: {e}", level="CRITICAL")
        logger.log("This operation may require administrator privileges. Try running with 'sudo'.", level="WARN")
        sys.exit(1)
    except Exception as e:
        logger.log(f"An unexpected error occurred: {e}", level="CRITICAL")
        sys.exit(1)
        
def show_current(args):
    try:
        display_id = int(args.display_id) if args.display_id else manager.get_main_display_id()
        if not args.display_id: logger.log(f"No ID provided, showing main display (ID: {display_id}).", level="DEBUG")

        info = manager._get_display_info(display_id)
        mode = info['current_mode']
        
        logger.log(f"Current mode for display '{info['name']}' (ID: {info['id']}):", level="INFO")
        hidpi_marker = f" @{mode['scale']}x (HiDPI)" if mode['hidpi'] else ""
        ...
        logger.log(f"Error getting current mode: {e}", level="CRITICAL")
        sys.exit(1)
